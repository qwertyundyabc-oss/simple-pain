...
public class PainHud implements HudElement {
	...
	private static void renderVignettePixels(DrawContext context, float ratio, float persistent,
		...
		for (int py = 0; py < texH; py++) {
			for (int px = 0; px < texW; px++) {
				...
				vignetteImage.setColorArgb(px, py, alphaByte << 24 | VIGNETTE_RGB);
			}
		}

		vignetteDynamicTexture.upload();
		context.drawTexture(RenderPipelines.GUI_TEXTURED, vignetteDynamicId,
			0, 0, 0, 0, sw, sh, texW, texH, texW, texH, 0xFFFFFFFF);
	}
	...
	private static void ensureVignetteTexture(int texW, int texH) {
		...
		vignetteDynamicTexture = new NativeImageBackedTexture(() -> "pain_vignette", vignetteImage);
