...
/** Optional Mod Menu integration. Client visual settings are applied on save. */
public final class ModMenuIntegration implements ModMenuApi {
	private static final Text TITLE = Text.translatable("pain_mechanic.config.title");
	private static final Text RESTART_NOTICE = Text.translatable("pain_mechanic.config.restart_notice");

	@Override
	public ConfigScreenFactory<?> getModConfigScreenFactory() {
		if (!isClothConfigAvailable()) {
			PainMechanic.LOGGER.warn("[Simple Pain] ModMenu configuration is unavailable because Cloth Config is not installed.");
			return null;
		}
	...
	private static boolean isClothConfigAvailable() {
		try {
			Class.forName("me.shedaniel.clothconfig2.api.ConfigBuilder", false,
	...
	private Screen createConfigScreen(Screen parent) {
		try {
			return createConfigScreenInternal(parent);
		} catch (LinkageError | RuntimeException e) {
			PainMechanic.LOGGER.error("[Simple Pain] Failed to create the ModMenu configuration screen. Check the Cloth Config version.", e);
			return parent;
		}
	}

	private Screen createConfigScreenInternal(Screen parent) {
		...
		addRestartNotice(builder.getOrCreateCategory(Text.translatable("pain_mechanic.config.category.general")), entries);
		addGameplay(builder.getOrCreateCategory(Text.translatable("pain_mechanic.config.category.gameplay")), entries, pending);
		addVisuals(builder.getOrCreateCategory(Text.translatable("pain_mechanic.config.category.visuals")), entries, pending);
	...
	private static void addRestartNotice(ConfigCategory category, ConfigEntryBuilder entries) {
		category.addEntry(entries.startTextDescription(RESTART_NOTICE).build());
	}
	...
	private static void addGameplay(ConfigCategory category, ConfigEntryBuilder e, PainConfig.PainConfigData c) {
		...
		category.addEntry(e.startBooleanToggle(key("shockEnabled"), c.shockEnabled)
			.setTooltip(Text.translatable("pain_mechanic.config.shockEnabled.tooltip"))
	...
	private static Text key(String name) {
		return Text.translatable("pain_mechanic.config." + name);
	}
}
