...
public final class PainClientTicker {
	private static final ResourceLocation SPEED_MODIFIER = com.painmechanic.PainMechanic.id("pain_speed");
	private static final ResourceLocation DAMAGE_MODIFIER = com.painmechanic.PainMechanic.id("pain_damage");
	private static final ResourceLocation ATTACK_SPEED_MODIFIER = com.painmechanic.PainMechanic.id("pain_attack_speed");
	private static final ResourceLocation ADRENALINE_SPEED_MODIFIER = com.painmechanic.PainMechanic.id("adrenaline_speed");
	private static final ResourceLocation ADRENALINE_DAMAGE_MODIFIER = com.painmechanic.PainMechanic.id("adrenaline_damage");
	private static final ResourceLocation ADRENALINE_ATTACK_SPEED_MODIFIER = com.painmechanic.PainMechanic.id("adrenaline_attack_speed");
	...
	public static void tick(Minecraft client) {
		...
		PainSystem.applyModifier(player, Attributes.MOVEMENT_SPEED, SPEED_MODIFIER, reduction);
		PainSystem.applyModifier(player, Attributes.ATTACK_DAMAGE, DAMAGE_MODIFIER, reduction);
		PainSystem.applyModifier(player, Attributes.ATTACK_SPEED, ATTACK_SPEED_MODIFIER, reduction);
		float adrenalineBonus = adrenaline ? 0.25f : 0f;
		PainSystem.applyBonusModifier(player, Attributes.MOVEMENT_SPEED, ADRENALINE_SPEED_MODIFIER, adrenalineBonus);
		PainSystem.applyBonusModifier(player, Attributes.ATTACK_DAMAGE, ADRENALINE_DAMAGE_MODIFIER, adrenalineBonus);
		PainSystem.applyBonusModifier(player, Attributes.ATTACK_SPEED, ADRENALINE_ATTACK_SPEED_MODIFIER, adrenalineBonus);
	...
	private static void updateAdrenalineState(LocalPlayer player) {
		boolean active = player.hasEffect(ModStatusEffects.ADRENALINE);
	...
	private static void tickDeathAudio(Minecraft client) {
		if (deathAudioPlayed) {
			return;
		}
		client.getSoundManager().stop();
		deathSound = new DeathSoundInstance(ModSounds.DEATH.get());
		client.getSoundManager().play(deathSound);
		deathAudioPlayed = true;
	}

	private static void resetDeathAudio(Minecraft client) {
		if (deathSound != null) {
	...
	private static void tickDying(Minecraft client) {
		...
		if (!dyingAudioStarted && now - dyingFadeStartMs >= DYING_FADE_MILLIS) {
			...
			endSound = null;
			dyingSound = new DyingSoundInstance(ModSounds.DYING.get());
	...
	public static float dyingOverlayAlpha() {
		...
		long elapsed = Util.getMillis() - dyingAudioStartMs;
		return Math.min(1f, Math.max(0f, elapsed / (float) DYING_AUDIO_MILLIS));
	}

	private static void resetDying(Minecraft client) {
		if (dyingSound != null) {
	 ...
	 */
	private static void updatePainDrone(Minecraft client, float pain, float maxHealth) {
		...
		if (ratio > threshold) {
			...
			if (painDrone == null) {
				painDrone = new PainDroneSoundInstance(ModSounds.PAIN_DRONE.get(), SoundSource.AMBIENT);
	...
	private static void updateCriticalSound(Minecraft client, boolean shock) {
		SoundEvent desired = shock ? ModSounds.PAIN_UNCONSCIOUS.get() : ModSounds.PAIN_CRITICAL.get();
		if (criticalSound == null || criticalSoundEvent != desired) {
			stopCriticalSound(client);
			criticalSoundEvent = desired;
			criticalSound = new PainDroneSoundInstance(desired, SoundSource.AMBIENT);
	...
	private static void stopCriticalSound(Minecraft client) {
		if (criticalSound != null) {
	...
	private static void updateEndSound(Minecraft client, boolean shouldPlay) {
		float step = 1f / (END_FADE_SECONDS * 20f);
		if (shouldPlay) {
			if (endSound == null) {
				endSound = new PainDroneSoundInstance(ModSounds.PAIN_END.get(), SoundSource.PLAYERS);
	...
	private static void stopEndSound(Minecraft client) {
		if (endSound != null) {
			client.getSoundManager().stop(endSound);
			endSound = null;
		}
	...
	private static void stopPainDrone(Minecraft client) {
		if (painDrone != null) {
