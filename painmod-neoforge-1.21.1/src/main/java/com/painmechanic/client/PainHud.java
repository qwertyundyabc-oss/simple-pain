...
public class PainHud {
	...
	/** 旧版 NativeImage.setPixelRGBA 使用 ABGR 像素格式，这里把 ARGB 的 VIGNETTE_RGB 转换好，避免红蓝互换。 */
	...
	private static DynamicTexture vignetteDynamicTexture;
	private static NativeImage vignetteImage;
	private static ResourceLocation vignetteDynamicId;
	...
	public static void renderVignette(GuiGraphics context) {
		...
		boolean restoreGuiShake = globalGuiContext == context && guiShakeAmplitude() > 0f;
		if (restoreGuiShake) {
	...
	private static void renderVignettePixels(GuiGraphics context, float ratio, float persistent,
		...
		for (int py = 0; py < texH; py++) {
			for (int px = 0; px < texW; px++) {
				...
				vignetteImage.setPixelRGBA(px, py, alphaByte << 24 | VIGNETTE_ABGR);
			}
		}

		vignetteDynamicTexture.upload();
		RenderSystem.enableBlend();
		context.blit(vignetteDynamicId,
	...
	private static void ensureVignetteTexture(int texW, int texH) {
		...
		if (vignetteDynamicTexture != null) {
			client.getTextureManager().release(vignetteDynamicId);
		}
		vignetteImage = new NativeImage(texW, texH, false);
		vignetteDynamicTexture = new DynamicTexture(vignetteImage);
		vignetteDynamicId = PainMechanic.id("vignette");
		client.getTextureManager().register(vignetteDynamicId, vignetteDynamicTexture);
		vignetteTexW = texW;
		vignetteTexH = texH;
	}
