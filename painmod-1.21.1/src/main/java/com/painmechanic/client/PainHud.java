...
public class PainHud {
	...
	public static void renderVignette(DrawContext context) {
		...
		// 透明度只由疼痛强度决定
		context.setShaderColor(1.0f, 1.0f, 1.0f, t);
		...
		// 1.21.x 普通 drawTexture 不启用 alpha 混合，需手动开启，否则透明区域会变成实体色块
		RenderSystem.enableBlend();
		context.drawTexture(VIGNETTE_TEXTURE,
			0, 0, sw, sh, 128 - halfRegion, 128 - halfRegion, region, region, 256, 256);
